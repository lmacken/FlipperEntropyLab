...
def main():
    parser = argparse.ArgumentParser(
        description="Monitor FlipperRNG debug logs")
    parser.add_argument(
    ...
    if args.list_ports:
        ...
        if ports:
            for i, port in enumerate(ports):
                print(
                    f"  {i+1}. {port['device']} - {port['description']} ({port['type']})")
        else:
    ...
    if not args.port:
        ...
        if ports:
            ...
            print(
                "❌ No Flipper Zero ports found. Use --list-ports to see available ports.")
