...
def basic_randomness_tests(data):
    ...
    print(f"Actual frequency range: {min(byte_counts.values())} - {max(byte_counts.values())}")
    
    # Chi-square test (simplified)
    chi_square = sum((count - expected_freq) ** 2 / expected_freq for count in byte_counts.values())
    ...
    print(f"Bit frequency test: {'PASS' if 0.49 < bit_ratio < 0.51 else 'FAIL'}")
    
    # Entropy estimation (Shannon entropy)
    entropy = -sum((count/len(data)) * (count/len(data)).bit_length() for count in byte_counts.values() if count > 0)
